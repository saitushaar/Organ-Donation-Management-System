...
Class Action {
	...
	function save_organdonor(){
		...
		$data .= ", relation = '$relation' ";
			...
			if(empty($id)){
				$save = $this->db->query("INSERT INTO organdonors set $data");
			}else{
				$save = $this->db->query("UPDATE organdonors set $data where id = $id");
			}
		if($save)
			return 1;
	}
	function delete_organdonor(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM organdonors where id = ".$id);
		if($delete){
			return 1;
		}
	}
